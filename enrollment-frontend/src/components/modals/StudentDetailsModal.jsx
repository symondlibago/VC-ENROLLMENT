...
const ApprovalAction = ({
  ...
  return (
    <div className="bg-white p-4 rounded-md border space-y-3">
      <Label htmlFor={`${roleName}-approval`} className="font-semibold text-gray-800">
        {roleLabel}
      </Label>
      <Select
      ...
      <div className="space-y-1">
        <Label htmlFor={`${roleName}-remarks`} className="text-sm text-gray-600">
          Remarks
        </Label>
        <Textarea
          id={`${roleName}-remarks`}
...
const StudentDetailsModal = ({ isOpen, onClose, studentId, currentUserRole }) => {
  ...
  // Store existing term payments so we don't overwrite them
  const [existingTermPayments, setExistingTermPayments] = useState([]);
    ...
    try {
      ...
      if (paymentRes && paymentRes.success) {
        ...
        if (studentLastUpdated > paymentLastUpdated) {
          ...
          // CASE C: CURRENT payment. 
          const allTermPayments = payment.term_payments || [];
          const currentYear = studentData.year;
          const currentSem = studentData.semester;
          const currentSY = studentData.school_year;

          const currentTermOnly = allTermPayments.filter(p =>
            p.year === currentYear &&
            p.semester === currentSem &&
            p.school_year === currentSY
          );

          setExistingTermPayments(currentTermOnly);
  ...
  useEffect(() => {
    ...
    const prevCredit = parseFloat(paymentData.advance_payment) || 0;
    ...
    const newTransactionAmount = parseFloat(transactionInput.amount) || 0;
    const balanceAfterDownPayment = newTotalAmount - newDiscountDeduction - paymentAmount - newTransactionAmount;

    let newRemainingAmount = balanceAfterDownPayment;
    let newAdvancePayment = 0;
    if (newRemainingAmount < 0) {
      newAdvancePayment = Math.abs(newRemainingAmount);
    ...
    setPaymentData(prev => ({
      ...
      advance_payment: manualEdit.advance_payment
        ? prev.advance_payment
        : (newAdvancePayment > 0 ? newAdvancePayment.toFixed(2) : prev.advance_payment),
    ...
    paymentData.previous_account, paymentData.registration_fee,
    paymentData.miscellaneous_fee, paymentData.other_fees,
    paymentData.payment_amount, paymentData.discount,
    paymentData.tuition_fee, paymentData.laboratory_fee,
    paymentData.bundled_program_fee, paymentData.total_amount,
    paymentData.discount_deduction, paymentData.term_payment,
    manualEdit, transactionInput.amount
  ]);
  ...
  const handleTransactionDateChange = (dateString) => {
    if (!dateString) {
      setTransactionInput(prev => ({ ...prev, payment_date: '' }));
      return;
    }
    const formattedDate = new Date(dateString).toISOString().split('T')[0];
    setTransactionInput(prev => ({ ...prev, payment_date: formattedDate }));
  };

  const handleSavePayment = async () => {
    const regFee = parseFloat(paymentData.registration_fee);
    const paymentAmount = parseFloat(paymentData.payment_amount); // Downpayment
    const totalAmount = parseFloat(paymentData.total_amount);

    const newTransAmount = parseFloat(transactionInput.amount);
    ...
    // Validation for New Transaction
    if (!isNaN(newTransAmount) && newTransAmount > 0) {
      if (!transactionInput.or_number) {
        setValidationModal({ isOpen: true, message: 'OR Number is required for the new transaction.' });
        return;
      }
      if (!transactionInput.payment_date) {
        setValidationModal({ isOpen: true, message: 'Date is required for the new transaction.' });
        return;
      }
    }
    ...
    try {
      ...
      // Construct Term Payments
      const termPaymentsPayload = existingTermPayments.map(({ id, created_at, updated_at, ...rest }) => rest);

      if (!isNaN(newTransAmount) && newTransAmount > 0) {
        termPaymentsPayload.push({
          amount: newTransAmount,
          or_number: transactionInput.or_number,
          payment_date: transactionInput.payment_date,
        });
      }
      ...
      const paymentPayload = {
        ...
        term_payments: termPaymentsPayload
      };
  ...
  const handleCreditSubject = async (subjectId) => {
    ...
    try {
      ...
      if (response.success) {
        setAlert({ isVisible: true, message: 'Subject credited successfully!', type: 'success' });
        fetchStudentDetails();
      }
    } catch (error) {
      setAlert({ isVisible: true, message: error.message || 'Failed to credit subject.', type: 'error' });
    } finally {
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input
                              type="number"
                              value={paymentData.previous_account}
                              onChange={(e) => handlePaymentInputChange('previous_account', e.target.value)}
                              className="w-full pl-8 pr-3 py-2 border-2 border-gray-300 rounded-md focus:border-blue-500 focus:outline-none"
                              placeholder="0.00"
                              min="0"
                              step="0.01"
                            />
                        ...
                        {[
                          ...
                          { label: 'Bundled Program Fee', key: 'bundled_program_fee' },
                        ].map((fee) => (
                          <div key={fee.key}>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                              {fee.label}
                            ...
                            <div className="relative">
                              ...
                              <input
                                type="number"
                                value={paymentData[fee.key]}
                                onChange={(e) => handlePaymentInputChange(fee.key, e.target.value)}
                                className="w-full pl-8 pr-3 py-2 border-2 border-gray-300 rounded-md focus:border-blue-500 focus:outline-none"
                                placeholder="0.00"
                                min="0"
                                step="0.01"
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {/* NEW: Add Payment Transaction Section */}
                        <div className="md:col-span-2 border-t pt-4 mt-2">
                          <h5 className="text-sm font-semibold text-gray-800 mb-2">
                            Add Payment Transaction (Term Payment)
                          </h5>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {/* OR # */}
                            <div>
                              ...
                              <input
                                type="text"
                                value={transactionInput.or_number}
                                onChange={(e) => handleTransactionChange('or_number', e.target.value)}
                                className="w-full px-3 py-2 text-gray-700 border-2 border-gray-300 rounded-md focus:border-blue-500 focus:outline-none"
                            ...
                            <div>
                              ...
                              <CustomCalendar
                                value={transactionInput.payment_date}
                        ...
                        {/* Calculated Term Payment */}
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">
                            Term Payment (Calculated)
                          </label>
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 text-sm">₱</span>
                            <input
                              type="number"
                              value={paymentData.term_payment}
                              onChange={(e) => handlePaymentInputChange('term_payment', e.target.value)}
                              className="w-full pl-8 pr-3 py-2 border-2 border-gray-300 rounded-md focus:border-blue-500 focus:outline-none"
                              placeholder="0.00"
                              min="0"
                              step="0.01"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      ...
                      {student.senior_high_school && (<div><p className="text-sm font-medium">Senior High School</p><p className="text-sm">{student.senior_high_school}</p><p className="text-xs text-gray-500">Completed: {student.senior_high_date_completed || 'Not specified'}</p></div>)}
                      {student.high_school_non_k12 && (<div><p className="text-sm font-medium">High School (Non-K12)</p><p className="text-sm">{student.high_school_non_k12}</p><p className="text-xs text-gray-500">Completed: {student.high_school_non_k12_date_completed || 'Not specified'}</p></div>)}
                      {student.college && (<div><p className="text-sm font-medium">College</p><p className="text-sm">{student.college}</p><p className="text-xs text-gray-500">Completed: {student.college_date_completed || 'Not specified'}</p></div>)}
                    </div>
                  </div>
