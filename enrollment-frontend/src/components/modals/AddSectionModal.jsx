// src/components/modals/AddSectionModal.jsx
...
const AddSectionModal = ({ isOpen, onClose, onSubmit, courses = [], sectionToEdit = null }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditMode && sectionToEdit) {
        setSectionName(sectionToEdit.name);
        const course = courses.find(c => c.id === sectionToEdit.course_id);
  ...
  }, [sectionToEdit, isEditMode, isOpen, courses]);
  ...
  const dropdownVariants = {
    ...
    exit: { opacity: 0, y: -10, scale: 0.95, transition: { duration: 0.15, ease: [0.23, 1, 0.32, 1] } }
  };

  const itemVariants = {
    hidden: { opacity: 0, x: -10 },
    visible: (i) => ({ opacity: 1, x: 0, transition: { delay: i * 0.05, duration: 0.2, ease: [0.23, 1, 0.32, 1] } })
  };
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {isEditMode ? 'Update the section details.' : 'Create a new section for students'}
                    </p>
                  </div>
                </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <motion.button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                    ...
                    {isDropdownOpen && (
                      ...
                      >
                        {courses.map((course, index) => (
                          <motion.button
                            key={course.id} type="button" variants={itemVariants} initial="hidden" animate="visible"
                            custom={index} onClick={() => handleCourseSelect(course)}
              ...
              <div className="flex space-x-3 pt-4">
                <Button type="button" variant="outline" onClick={handleCancel} className="flex-1 liquid-button">
                  Cancel
                </Button>
                <Button
                  type="submit" className="flex-1 gradient-primary text-white liquid-button"
