...
import { BookUser, Search, Eye, Users, Hash, User, GraduationCap, CalendarDays, Settings2, SlidersHorizontal, Loader2, Save, FileDown, ChevronDown } from 'lucide-react';
...
import ExportGradingSheet from '@/components/layout/ExportGradingSheet';

// ---------------------------------------------------------------------------
// SECTION 1: Student list view
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SECTION 2: Grade submission date manager
// ---------------------------------------------------------------------------
const GradeSubmissionManager = () => {
  ...
  const handleDateChange = (periodKey, dateType, valueFromCalendar) => {
    ...
    setPeriods(prev => ({ ...prev, [periodKey]: { ...prev[periodKey], [dateType]: formattedDate } }));
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {Object.entries(periodNames).map(([key, name]) => {
            ...
            const popupPosition = (key === 'semifinal' || key === 'final' || key === 'enrollment') ? 'above' : 'below';
            ...
            return (
              <div key={key} className={`p-4 border rounded-lg grid grid-cols-1 md:grid-cols-4 items-center gap-4 ${key === 'enrollment' ? ' border-gray-200' : ''}`}>
                ...
                <div className="md:col-span-1">
                  ...
                  <CustomCalendar value={periodData.start_date} onChange={(date) => handleDateChange(key, 'start_date', date)} placeholder="Select start date" position={popupPosition} />
                </div>
                <div className="md:col-span-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">End Date</label>
                  <CustomCalendar value={periodData.end_date} onChange={(date) => handleDateChange(key, 'end_date', date)} placeholder="Select end date" position={popupPosition} />
                </div>
              </div>
            );
          })}
...
// ---------------------------------------------------------------------------
// SECTION 3: Export dropdown button (sits beside the toggle)
// ---------------------------------------------------------------------------
const ExportDropdown = ({ onExportGrades }) => {
  ...
  return (
    <div ref={ref} className="relative">
      <Button
        variant="outline"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 border-green-600 text-green-700 hover:bg-green-700 hover:text-white cursor-pointer transition"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-xl shadow-xl z-40 overflow-hidden"
          >
            <button
              onClick={() => { onExportGrades(); setOpen(false); }}
              className="w-full flex items-center gap-3 px-4 py-3 text-sm text-left text-gray-700 hover:bg-green-50 hover:text-green-800 transition"
            >
              <FileDown className="w-4 h-4 text-green-600" />
              <div>
                <p className="font-medium">Export Grading Sheet</p>
                <p className="text-xs text-gray-400">By instructor, subject &amp; section</p>
              </div>
            </button>
          </motion.div>
        )}
...
// ---------------------------------------------------------------------------
// SECTION 4: Main Grades component
// ---------------------------------------------------------------------------
const Grades = () => {
  ...
  return (
    <>
      ...
      {/* Export grading sheet modal */}
      {showExportModal && (
        <ExportGradingSheet onClose={() => setShowExportModal(false)} />
      )}
      ...
      >
        <motion.div variants={itemVariants}>
          <div className="gradient-soft rounded-2xl p-8 border border-gray-100 flex items-center justify-between gap-4 flex-wrap">
            {/* Title */}
            <div>
              <h1 className="text-3xl font-bold heading-bold text-gray-900 mb-2 flex items-center">
                {activeTab === 'gradebook'
                  ...
                  : <SlidersHorizontal className="w-8 h-8 text-[var(--dominant-red)] mr-3" />}
              ...
              <p className="text-gray-600 text-lg">
                {activeTab === 'gradebook'
                  ...
                  : 'Set the submission dates for each grading period.'}
              </p>
            </div>

            {/* Right side: Export dropdown + Toggle */}
            <div className="flex items-center gap-3">
              {/* Export dropdown — visible on gradebook tab only */}
              {activeTab === 'gradebook' && (
                <ExportDropdown onExportGrades={() => setShowExportModal(true)} />
              )}
              ...
              <div className="relative bg-gray-100 rounded-2xl p-1 inline-flex">
                ...
                <motion.button
                  ...
                  className={`relative z-10 p-3 rounded-xl transition-colors duration-300 ${activeTab === 'gradebook' ? 'text-[var(--dominant-red)]' : 'text-gray-600'}`}
                  title="Gradebook"
                >
                ...
                <motion.button
                  ...
                  className={`relative z-10 p-3 rounded-xl transition-colors duration-300 ${activeTab === 'management' ? 'text-[var(--dominant-red)]' : 'text-gray-600'}`}
                  title="Management"
                >
